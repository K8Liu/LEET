...
const bstMinMax = root => {
  let current = root;
  //initiate leftmost(min) variable
  let left = current.left;
  //initiate rightmost(max) variable
  let right = current.right;
  //traverse the tree on each side; get the leftmost value and the rightmost value
  if (left.left.value) {
    left = left.left;
  }
  ...
  //return the difference of rightmost minus leftmost
